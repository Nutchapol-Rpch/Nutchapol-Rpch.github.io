...
function App() {
  ...
  const addItem = () => {
    ...
    for (let i = 0; i < dataItems.length; i++) {
      if (dataItems[i].name === newItem.name && dataItems[i].ppu === newItem.ppu) {
        dataItems[i].qty = (parseInt(dataItems[i].qty) + parseInt(newItem.qty)).toString();
        sim = true;
        ...
        <Col md={4} style={{ backgroundColor: "#e4e4e4" }}>
          ...
          <Row>
            <Col>
              ...
              <Form.Control type="number" ref={disRef} defaultValue={0} onChange={e => setDis(disRef.current.value)}/>
            </Col>
          </Row>
          <hr />
